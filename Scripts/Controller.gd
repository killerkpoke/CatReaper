...
export(int) var JUMP_HEIGHT = 9 #the number of pixels a character can jump
export(float) var timeToJumpApex = .8 #the amount of time to reach the peak of the jump Higher = slower jump
export(int) var MAX_SPEED = 300 #Max speed in pixels per second
...
export var player_cheatdeath = 9
onready var hearths = [
	$Camera2D/Panel/Health/HealthSprites,
	$Camera2D/Panel/Health/HealthSprites2,
	$Camera2D/Panel/Health/HealthSprites3
]
...
func _ready():
	...
	$Camera2D/Panel/CheathDeath/CDLabel.text = str(player_cheatdeath)
...
#	if joystick.ongoing_drag == -1:    
	...
	if (Input.is_action_just_pressed("ui_accept") or Input.is_action_just_pressed("ui_up")) and JumpOn:
		jumpPressed = true
		jumpTime()
		if canjump == true:
			state = JUMP
	...
	if (Input.is_action_just_pressed("ui_accept") or Input.is_action_just_pressed("ui_up")) and velocity.y < minJump:
		velocity.y = 0
	
	#if Input.is_action_just_pressed("attack") and AttackOn:
		#state = ATTACK -> Not restricting to state for smoother inputs
		#print('shoot')
...
func _on_Area2D_area_entered(area):
	if area.name == "EnemyHitbox":
		...
		player_health -= 1
		hearths[player_health].visible = false
		$AudioStreamPlayer.play()
		if player_health <= 0:
			if player_cheatdeath <= 0:
				print('DEAD')
				yield(get_tree().create_timer(0.2),"timeout")
